// P2-04 模型调用追溯台 UI 契约测试 Harness（零 npm 依赖）。
// 用最小 DOM shim 在 Node 中真实执行 frontend/index.html 的内联脚本，
// 再由 driver 驱动调用列表分页/概要白名单/父子链反查/文本增量断线续传与终态覆盖/
// 图片真实状态/宿主 RBAC 详情门禁/403/404/409/422/503/项目切换与刷新隔离。
// 用法: node model_calls_ui_harness.mjs <index.html 路径> <driver.js 路径>
...
// 区域感知动态缓存：#content 全量重渲、#obs-root / #hist-root / #trace-root 定向重渲分别持有独立缓存。
...
const contentEl = makeEl('#content', () => { contentCache.clear(); obsCache.clear(); histCache.clear(); traceCache.clear(); obsRootEl = null; histRootEl = null; traceRootEl = null; });
...
globalThis.document = {
  ...
  querySelectorAll(sel) {
    ...
    if (sel === '[data-trace-call]') {
      const out = []; const re = /data-trace-call="([^"]*)"/g; let m;
      while ((m = re.exec(contentEl.innerHTML))) { const el = makeEl('trace-call'); el.dataset.traceCall = m[1]; out.push(el); }
      return out;
    }
    if (sel === '#trace-root [data-trace-node]') {
      const out = []; const host = traceRootEl; if (!host) return out;
      const re = /data-trace-node="([^"]*)"/g; let m;
      while ((m = re.exec(host.innerHTML))) { const el = makeEl('trace-node'); el.dataset.traceNode = m[1]; out.push(el); }
      return out;
    }
    return [];
  },
...
// ---- FakeEventSource：P2-03 监督台在建工程视图时会建连，保持可用以免干扰 P2-04 场景 ----
const eventSources = [];
...
globalThis.fetch = async (url, options = {}) => {
  ...
  if (u.includes('/model-calls') && method === 'GET') {
    const impl = globalThis.__ui.modelCallsImpl;
...
// ---- 在同一函数作用域内依次执行页面脚本与 driver（driver 可访问页面内部 state/renderProject/trace* 函数） ----
