// P1-07 轮数上限分流 UI 契约测试 Harness（零 npm 依赖）。
// 用最小 DOM shim 在 Node 中真实执行 frontend/index.html 的内联脚本，
// 再由 driver 文件驱动 waiting_quality_disposition 相位的三个分流按钮，
// 验证真实点击产生的请求体、continue 费用确认/取消路径、重复点击保护与零控制台错误。
// 用法: node quality_disposition_ui_harness.mjs <index.html 路径> <driver.js 路径>
