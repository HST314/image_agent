// P2-06 健康与诊断台 UI 契约测试 Harness（零 npm 依赖）。
// 用最小 DOM shim 在 Node 中真实执行 frontend/index.html 的内联脚本，
// 再由 driver 驱动 liveness/readiness 分层渲染、六组件映射、离线/未配置提示、
// 503 关键与 200 局部退化、trace 关联、reader/operator/admin 诊断门禁、
// 敏感字段不落公共 DOM、限频轮询去重、错误恢复与页面切换迟到响应隔离。
// 用法: node health_ui_harness.mjs <index.html 路径> <driver.js 路径>
...
const contentEl = makeEl('#content', () => { contentCache.clear(); obsCache.clear(); histCache.clear(); traceCache.clear(); stgsCache.clear(); hlthCache.clear(); obsRootEl = null; histRootEl = null; traceRootEl = null; stgsRootEl = null; hlthRootEl = null; });
...
function dynamicById(id) {
  ...
  if (hlthRootEl && re.test(hlthRootEl.innerHTML)) {
    if (!hlthCache.has(id)) hlthCache.set(id, makeEl('hlth#' + id));
    return hlthCache.get(id);
  }
  ...
  if (re.test(contentEl.innerHTML)) {
    ...
    if (id === 'hlth-root') {
      if (!hlthRootEl) hlthRootEl = makeEl('#hlth-root', () => { hlthCache.clear(); });
      return hlthRootEl;
    }
...
// ---- FakeEventSource：P2-03 监督台在建工程视图时会建连，保持可用以免干扰 P2-06 场景 ----
const eventSources = [];
...
const TRACE_RE = /^trace_[a-f0-9]{32}$/;
const mkTrace = (seed) => 'trace_' + String(seed).replace(/[^a-f0-9]/g, 'a').padEnd(32, '0').slice(0, 32);
globalThis.fetch = async (url, options = {}) => {
  fetchCalls.push({ url: String(url), options });
  const u = String(url);
  const method = String(options.method || 'GET').toUpperCase();
  if (u.endsWith('/api/health/live')) {
    const impl = globalThis.__ui.liveImpl;
    if (impl) return impl(u, options);
    return { ok: true, status: 200, json: async () => ({ status: 'alive', checked_at: '2026-08-08T01:00:00+00:00', trace_id: mkTrace('1') }) };
  }
  if (u.endsWith('/api/health/ready')) {
    const impl = globalThis.__ui.readyImpl;
    if (impl) return impl(u, options);
    return { ok: true, status: 200, json: async () => globalThis.__ui.readyPayload() };
  }
  if (u.includes('/api/internal/diagnostics/')) {
    const impl = globalThis.__ui.diagImpl;
    if (impl) return impl(u, options);
    return { ok: false, status: 403, json: async () => ({ detail: { code: 'DIAGNOSTICS_FORBIDDEN', message: '无内部诊断权限。' } }) };
  }
...
globalThis.__ui = {
  ...
  projectsImpl: null,
  readyPayload: () => ({
    status: 'ready', checked_at: '2026-08-08T01:00:05+00:00', trace_id: mkTrace('2'),
    components: Object.fromEntries(['model_router', 'worker', 'queue', 'storage', 'event_append', 'asset_proxy'].map((n) => [n, {
      status: 'healthy', error_code: 'OK', checked_at: '2026-08-08T01:00:05+00:00', business_impact: n + ' 可用',
    }])),
  }),
...
// ---- 在同一函数作用域内依次执行页面脚本与 driver（driver 可访问页面内部 state/hlthUi/hlth* 函数） ----
