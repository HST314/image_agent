// P1-10 统一错误呈现 UI 契约测试 Harness（零 npm 依赖）。
// 用最小 DOM shim 在 Node 中真实执行 frontend/index.html 的内联脚本，
// 再由 driver 文件驱动 AsyncJob/checkpoint 稳定错误对象的统一消费：
// suggested_action 五类映射、429/Retry-After、可重试与不可重试差异、
// 部分槽位恢复、刷新/worker 重启一致、上限停重试、取消与人工等待差异、
// 旧错误对象兼容、敏感详情转义、重复点击单请求、422 与 job failed 边界。
// 用法: node error_ui_harness.mjs <index.html 路径> <driver.js 路径>
...
const dynamicCache = new Map(); // 由 #content innerHTML 驱动的动态元素
function makeCtx() {
  const ctx = {
    calls: [],
...
globalThis.fetch = async (url, options = {}) => {
  ...
  if (u.includes('/jobs/') && u.endsWith('/cancel') && method === 'POST') {
    const impl = globalThis.__ui.cancelImpl;
    if (impl) return impl(u, options);
    return { ok: true, status: 200, json: async () => ({ status: 'cancel_requested' }) };
  }
  if (u.includes('/jobs/') && method === 'GET') {
    const impl = globalThis.__ui.jobImpl;
    if (impl) return impl(u, options);
    return { ok: true, status: 200, json: async () => ({ status: 'succeeded', progress: { completed: 1, total: 1, unit: 'workflow' } }) };
  }
  ...
  if (u.includes('/retry') && method === 'POST') {
    const impl = globalThis.__ui.retryImpl;
    if (impl) return impl(u, options);
...
globalThis.__ui = {
  ...
  getView: null, advanceImpl: null, jobImpl: null, cancelImpl: null, retryImpl: null,
  setDPR(v) { globalThis.devicePixelRatio = v; },
  storage: { local: globalThis.localStorage, session: globalThis.sessionStorage },
};

// ---- 在同一函数作用域内依次执行页面脚本与 driver（driver 可访问页面内部 state/renderProject/jobUi） ----
...
}).catch((err) => { writeSync(2, String((err && err.stack)) + '\n'); process.exit(2); });
