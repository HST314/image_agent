// P1-09 交付与人工回传 UI 契约测试 Harness（零 npm 依赖）。
// 用最小 DOM shim 在 Node 中真实执行 frontend/index.html 的内联脚本，
// 再由 driver 文件驱动 completed/delivery_frozen 相位的交付区域，
// 验证未生成/生成中/失败可重试/待回传/已回传状态、稳定幂等键、
// checkpoint 无关渲染、重复点击防护与 409/网络失败可恢复状态。
// 用法: node delivery_ui_harness.mjs <index.html 路径> <driver.js 路径>
...
const dynamicCache = new Map(); // 由 #content innerHTML 驱动的动态元素（如 #dlv-actor）
function makeCtx() {
  const ctx = {
    calls: [],
...
globalThis.fetch = async (url, options = {}) => {
  ...
  if (u.includes('/delivery/generate') && method === 'POST') {
    const impl = globalThis.__ui.deliveryGenerateImpl;
    if (impl) return impl(u, options);
    return { ok: false, status: 503, json: async () => ({ detail: '后端能力暂不可用：未配置说明生成。' }) };
  }
  if (u.includes('/delivery/return') && method === 'POST') {
    const impl = globalThis.__ui.deliveryReturnImpl;
    if (impl) return impl(u, options);
    return { ok: false, status: 409, json: async () => ({ detail: 'Delivery 版本不存在。' }) };
  }
  if (u.includes('/delivery') && method === 'GET') {
    const impl = globalThis.__ui.deliveryGetImpl;
    if (impl) return impl(u, options);
    return { ok: false, status: 409, json: async () => ({ detail: '设计说明尚未生成，当前没有可读取的 Delivery。' }) };
  }
...
globalThis.__ui = {
  ...
  getView: null, deliveryGetImpl: null, deliveryGenerateImpl: null, deliveryReturnImpl: null,
  setDPR(v) { globalThis.devicePixelRatio = v; },
  storage: { local: globalThis.localStorage, session: globalThis.sessionStorage },
};

// ---- 在同一函数作用域内依次执行页面脚本与 driver（driver 可访问页面内部 state/renderProject/deliveryUi） ----
